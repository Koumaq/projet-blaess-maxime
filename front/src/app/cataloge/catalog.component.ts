...
import {AjouterPro} from "../shared/action/cart-action";
...
export class CatalogComponent implements OnInit, AfterViewInit {
  ...
  ajouterPanier(p: Product): void {
    this.store.dispatch(new AjouterPro(p));
  }

  ngAfterViewInit() {
    this.searchField = fromEvent(this.input.nativeElement, "keyup");

    const ToutCataloge = this.catalogService.getProducts().pipe(
      catchError(() => {
        return [];
      })
    );
